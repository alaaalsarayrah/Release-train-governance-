...
export default async function handler(req, res) {
  try {
    if (req.method === 'GET') {
      ...
      return res.status(200).json({ dependencies: session.dependencies || [] })
    }
    ...
    if (req.method === 'POST') {
      ...
      await saveDependencyRecords(sessionId, records)
      await writePlanningLog({
        sessionId,
        stage: 'Dependency Analyst',
        actor,
        message: 'Dependency records saved',
        eventType: 'info',
        details: { count: records.length }
      })

      const session = await getPlanningSessionWithChildren(sessionId)
      return res.status(200).json({ success: true, dependencies: session?.dependencies || [] })
    }
