...
function findTeamAssigneeCandidates(setup, teamName, options = {}) {
  ...
  const teamScopes = targetTeam ? [targetTeam, ...teams.filter((t) => t !== targetTeam)] : teams
...
export async function resolveAssigneeCandidatesForAdo({ config, teamName }) {
  ...
  const teamCandidates = findTeamAssigneeCandidates(setup, teamName, { allowNameFallback: true })
