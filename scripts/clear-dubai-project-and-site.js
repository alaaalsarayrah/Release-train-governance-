...
const dbPath = path.join(rootDir, 'data', 'requests.db')
const uploadsDir = path.join(rootDir, 'public', 'uploads')
const adoConfigPath = path.join(rootDir, 'public', '.ado-config.json')
...
async function fetchProjectWorkItemIds({ org, project, pat }) {
  ...
  return (response.workItems || []).map((item) => item.id).filter(Boolean)
}

async function deleteWorkItem({ org, project, pat, id }) {
  return adoRequest({
    org,
    project,
    pat,
...
async function clearAdoProjectData() {
...
async function main() {
  try {
    const site = await clearSiteData()
    const ado = await clearAdoProjectData()
